...
app.post('/api/signup', async (req, res) => {
    ...
    if (!username || !password) {
        return res.status(400).json({ error: 'Server Message: Username and password required.' });
    }
    ...
    pool.query(checkUsernameQuery, checkUsernameValues, async (checkUsernameErr, checkUsernameResult) => {
        if (checkUsernameErr) {
            ...
            return res.status(500).json({ error: 'Server Message: Internal server error' });
        }

        // If count is greater than 0, username already exists
        if (checkUsernameResult[0].count > 0) {
            return res.status(409).json({ error: 'Server Message: Username already exists' });
        }
...
app.post('/api/login', async (req, res) => {
    ...
    pool.query(sql, values, async (err, result) => {
        ...
        if (result.length === 0) {
            return res.status(401).json({ error: 'Server Message: Invalid username or password' });
        }
        ...
        try {
            ...
            if (passwordMatch) {
                return res.status(200).json({ message: 'Server Message: Login successful' });
            } else {
                return res.status(401).json({ error: 'Server Message: Invalid username or password' });
            }
        } catch (error) {
